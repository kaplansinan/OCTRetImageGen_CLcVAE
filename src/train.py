...
def build_decoder(emb_shape=128, latent_dim=128, upscale_factor=4):
    ...
    #             activation='relu')(x) #128
    #x = layers.Conv2DTranspose(
    #            filters=8, kernel_size=3, strides=2, padding='same',
    #             activation='relu')(x)#256
    # # output_image = layers.Conv2D(3,  kernel_size=3, padding="same")(x)
    #output_image = layers.Conv2D(3, kernel_size=3, activation='tanh', padding="same")(x)
...
econder_path = "D:/Learning/XAI_AUTOENCODERS_2022/output/20220614-161700_cvae_subpixel_kl_reco_feat_loss_cnv/model_encoder_20.h5"
decoder_path = "D:/Learning/XAI_AUTOENCODERS_2022/output/20220614-161700_cvae_subpixel_kl_reco_feat_loss_cnv/model_decoder_20.h5"

model.encoder.load_weights(econder_path)
...
train_df = train_df[train_df['label']=='CNV']
# train_df = train_df.groupby('label').apply(pd.DataFrame.sample, frac=0.5).reset_index(drop=True)
...
LOG_DIR = os.path.join('../output',datetime.now().strftime("%Y%m%d-%H%M%S")+'_cvae_subpixel_kl_reco_feat_loss_CNV')
...
#     for test_x in tqdm(tf_train_set,"running tests"):
#         tmp_x_test_images, tmp_x_test_labels = test_x
#         loss(compute_loss(model, tmp_x_test_images, tmp_x_test_labels))
